...
/***        Type Definitions                                              ***/
/****************************************************************************/
typedef void (*TNotifyMBWriteSingleRegEvent)(unsigned int);
...
class ModbusSlave {
	...
	public:
		...
		TNotifyMBWriteSingleRegEvent	writesingleregister;
		TNotifyWriteCoilEvent			writecoil;

		/*
		 * writeRegCoil -> Escribe un registro en el banco de registros COIL.
		 *
		 * Genera un evento para activar, actualizar la salida correspondiente.
		 */
		void writeRegCoil(unsigned int write_addr, int reg_val, uint16_t *regs);

		/*
		 * writeSingleRegister -> Escribe un registro en el banco de registros "Analog Output Holding Register"
		 *
		 * Genera un evento para activar, actualizar la salida correspondiente.
		 */
		void writeSingleRegister(unsigned int write_addr, unsigned int reg_val, uint16_t *regs);
		...
		int read_Analog_registers(unsigned int Func, unsigned int start_addr, unsigned char reg_count, uint16_t *regs);
...
extern uint16_t			*A_Regs;
extern uint8_t 			N_ARegs;

//Analog Output Holding Register
extern uint16_t			*E_Regs;
extern uint8_t 			N_ERegs;

//Discrete Output Coils
extern uint16_t			*C_Regs;
extern uint8_t 			N_CRegs;

//Discrete Input Contacts
extern uint16_t			*D_Regs;
extern uint8_t			N_DRegs;
