...
enum { 
	...
	FC_READ_REG_AO		= 	0x03,   //Read contiguous block of holding registers
...
uint16_t	*A_Regs;						//Analog holding Registers
uint8_t		N_ARegs;
uint16_t	*E_Regs;						//Analog Input Registers (Registros de entrada)
uint8_t		N_ERegs;
uint16_t 	*C_Regs; 						//Discrete Output Coils
uint8_t		N_CRegs;
uint16_t 	*D_Regs;						//Discrete Input Contacts
uint8_t		N_DRegs;
...
int ModbusSlave::send_reply(unsigned char *query, unsigned char string_length) 
{
	...
	if (txenpin > 1) { // set rs485 to speak mode
		UCSR0A=UCSR0A |(1 << TXC0);
		digitalWrite( txenpin, HIGH);
		delay(1);
	}

	//domoBoard.ToggleSalida(&domoBoard.Rele_2);
	modbus_reply(query, string_length);
	string_length = string_length + 2;

	//Serial.write(query,string_length);
	Serial485->write(query,string_length);

	delay(10);
	if (txenpin > 1) {// set rs485 to listen mode
		while (!(UCSR0A & (1 << TXC0)));
...
*/
	...
	//sprintf(msg,"start_addr : %d --> regs_num : %d --> regs_size : %d", start_addr, regs_num, regs_size);
	//Serial.println(msg);
...
*/
        if(writesingleregister != NULL){
        	...
        	writesingleregister(write_addr);
        }

        Serial.flush();

        status = send_reply(packet, RESPONSE_SIZE);
        //domoBoard.SetSalida(&domoBoard.Rele_1, ON);
...
int ModbusSlave::write_coil(unsigned int write_addr, unsigned char *query, uint16_t *regs){
	...
	writeRegCoil(write_addr, reg_val, regs);
...
/*
 * Modifica el valor del registro ModBus asociado a un Coil (Elemento Digital salida)
 */
void ModbusSlave::writeRegCoil(unsigned int write_addr, int reg_val, uint16_t *regs){
	regs[write_addr] = (int) reg_val;
...
/************************************************************************
 * 
 * writeVal_single_register(write_addr, val, registers_array)
 *
 * Write a single int val into a single holding register of the slave.
 *
 *************************************************************************/

void ModbusSlave::writeSingleRegister(unsigned int write_addr, unsigned int reg_val, uint16_t *regs)
{
	regs[write_addr] = (int) reg_val;
...
int ModbusSlave::read_discrete_registers(unsigned int Func, unsigned int start_addr, unsigned char reg_count, uint16_t *regs)
{
	...
	for (i = start_addr; i < (start_addr + (unsigned int) reg_count);i++) {
		//packet[packet_size] |= (regs[i] & 0x0001) << regnum;
		packet[packet_size] |= (regs[i] && 0xFF00) << regnum;
...
int ModbusSlave::read_Analog_registers(unsigned int Func, unsigned int start_addr,unsigned char reg_count, uint16_t *regs)
{
	...
	unsigned int i;
	unsigned char packet[MAX_MESSAGE_LENGTH];
	//unsigned char packet[10] = {0x01,0x03,0x04,0x00,0x07,0x00,0x08,0x4A,0x34};

	//Serial.print(domoboard.BOTON1.sSensor.name);
	//Serial.print("  : ");
/*
	Serial.println("read_holding_registers");
...
int ModbusSlave::update()
{
	...
	//exception = 0x05;
	//Serial485->print("exception : ");
	//Serial485->println(exception);
	...
	case FC_READ_REG_DI:
		//Serial.println("FC_READ_REG_DI");
		return read_discrete_registers(FC_READ_REG_DI, start_addr,query[REGS_L],D_Regs);
		break;

	case FC_READ_REG_AO:
		return read_Analog_registers(query[FUNC], start_addr,query[REGS_L],A_Regs);
		break;

	case FC_READ_INPUT_REGS:
		//Serial.println("FC_READ_REG_AO");
		return read_Analog_registers(query[FUNC], start_addr,query[REGS_L],E_Regs);
		break;
...
//		return preset_multiple_registers(start_addr,query[REGS_L],query,A_Regs);
	...
	case FC_WRITE_REG:
		//Serial.println("FC_WRITE_REG");
		write_single_register(start_addr,query,A_Regs);
		break;

	case FC_WRITE_COIL:
		write_coil(start_addr,query,C_Regs);
		break;
	}
